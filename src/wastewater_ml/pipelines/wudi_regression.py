...
class Regression:
    ...
    def fit(self, features, labels):
        ...
        print(Fore.GREEN + "Completed Run [", "\u2713", "]\n")
        self.end = time.time()
    ...
    def cate_features(self, categories=False, *args, **kwargs):

        if not categories:
            pass

        if categories:
            perm_importance_mean, sorted_idx = self.preprocess.category_feature_imp(self.features, self.X_train, self.y_train, model=self.regressor)
            return perm_importance_mean, sorted_idx